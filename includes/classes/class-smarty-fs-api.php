...
class Smarty_Fs_API {
    ...
    private function make_api_request($endpoint, $method = 'GET', $query_params = []) {
        ...
        if (is_wp_error($response)) {
            //_fs_write_logs('API request error: ' . $response->get_error_message());
            return false;
        }
    
        //_fs_write_logs('API request response: ' . print_r($response, true));
